...
impl Character {
    ...
    pub fn update(&mut self, delta_time: f32) {
        ...
        if self.velocity.y == 0.0 {
            self.animation_position += 2.0 * delta_time;
        } else {
    ...
    pub fn draw(&self) -> (&ugli::Texture, Vec3<f32>, Vec2<f32>, Size) {
        let (animation, origin, size): (&Animation, Vec2<f32>, f32) = if self.velocity.y == 0.0 {
            (&self.assets.fall, vec2(0.5, 0.5), 0.7)
        } else {
            (&self.assets.run, vec2(0.5, 0.0), 1.0)
        };
